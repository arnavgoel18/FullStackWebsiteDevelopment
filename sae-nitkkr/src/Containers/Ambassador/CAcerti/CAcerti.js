...
function Cacerti() {
  ...
  async function getDbData() {
    ...
    let folder = ref(storage, `campusAmbassador//${imgNo}.jpg`);
    setImg(imgNo);
   ...
   async function getallurl(){
     const all = collection(db, "collegeRepresentatives");
  ...
  return (
    <>
      <NavBar />
      <div className='App'>
        <h2>Enter Phone no (for CA certificates)</h2>
        <input
          type='tel'
          className='imgNumber'
          style={{
